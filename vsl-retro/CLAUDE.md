...
- All clickable things must be real `<a>`/`<button>` elements with labels. The phone PalmPilot launcher is generated from tools.json in `build()`; never hand-edit it.
...
- Resize under 700 px: the PalmPilot must appear with all 13 icons; its screen is `left:34 top:148 720×944` in a 768×1376 scene — do not move it.
