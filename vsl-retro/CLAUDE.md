...
- Tool data belongs in `js/tools.json` (lab) and `js/home.json` (homepage), never hard-coded in HTML. An entry with `"app"` instead of `href` is a built-in program from `js/extras.js` (loaded on first use, never up front); keep extras text-only, with sound only on an explicit button. `home.html` is `index.html` with different copy; when you change chrome in one, change the other.
